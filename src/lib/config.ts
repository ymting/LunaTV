...
// 内置默认配置序列化结果，用于空配置初始化与存量实例自动补充
export const DEFAULT_CONFIG_FILE = JSON.stringify(defaultConfig);
...
export async function getConfig(): Promise<AdminConfig> {
  ...
  } else if (!adminConfig.ConfigFile || !adminConfig.ConfigFile.trim()) {
    // 存量实例未配置任何源：注入内置默认配置，避免升级后空源
